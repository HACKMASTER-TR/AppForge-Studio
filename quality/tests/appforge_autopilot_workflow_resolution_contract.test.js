...
test(
  ...
  async () => {
    ...
    assert.doesNotMatch(
      source.slice(source.indexOf("def required_main_workflows("), source.indexOf("def wait_required_main_ci(")),
      /worker-image\.yml|source-worker-image\.yml|conversion-smoke\.yml/
    );

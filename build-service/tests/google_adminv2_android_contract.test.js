...
test('owner entitlement requires verified ephemeral Google identity, never a legacy AppForge email', async () => {
...
test('only explicit Google button can start credential flow; server verifies before remembering admin token', async () => {
  ...
  assert.match(google, /rememberVerifiedGoogleAdmin\(idToken, expiresAt\)/);
...
test('home entry replaces legacy login; Terminal and Second Brain require owner', async () => {
  ...
  assert.doesNotMatch(ops, /SecureAccountStore/);
